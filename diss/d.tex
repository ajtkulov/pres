...
\begin{itemize}
  \item software/data/ML-engineer, fp
  \item спортивное ЧГК
  \item шахматы, $Elo = 2050 \ldots 2100, Elo_{max} = 2118$, lichess
  \item бег, марафон: 3h 22m, $\frac{1}{2}$ марафон: 1h 37m, 10k: 40m 38s
...
Не политика. Quis custodiet ipsos custodes?
...
\begin{itemize}
  \item карбункул, нарезка. \href{https://dissernet.org/expertise/DanilinaLYu2012}{https://dissernet.org/expertise/DanilinaLYu2012}, \href{https://www.dissernet.org/expertise/RamazanovaKK2011}{https://www.dissernet.org/expertise/RamazanovaKK2011}
...
Зачем злодеи это делают? Списывание, торговля, криминал, продвижение по службе.
}
...
\begin{itemize}
  ...
  \item Платные соавторы, продажа места в соавторах.
\end{itemize}
...
Ретрагирование статьи. Хищнические журналы. Paper mill.

\vspace{3mm}

\pause

Торговля ссылками, прокачка H-index.
}
...
\vspace{3mm}
  11.5К сильное подозрение, 3К экспертиз, 2000+ ЗОЛУСов. \par
\vspace{3mm}
  250 (из 270) ЗОЛУСов пройдено в 2021 году. 90\% ЗОЛУСов удовлетворены.
}
...
\item cyberleninka (3.5M статей, 200Гб текста)
...
\frame {
  ...
  Версия 1. длинные <<предложения>>. Хеширование, 64 бита. 500М, sampling 10\%
...
Сейчас найдено порядка 20 случаев манипуляций с данными и 200+ кражи.
...
\vspace{3mm}
  ...
  Прогоны по кандидатам в ГД РФ. Порядка 20\% попаданий.
}
...
\begin{itemize}
  ...
  \item Табличный плагиат, точное совпадение, манипуляции с данными

  \item Обновление источников

  \item Фантомы, VIP, Фуфломицины

  \item Переводной плагиат
\end{itemize}
...
\begin{itemize}
  ...
  \item
  155К Диссертаций, 2000 - н.в. Говорят на своем и ином языке. 

  \item
  Без общественного контроля происходят темные вещи.
...
\frame {
  \frametitle {Переводной плагиат}

\begin{itemize}
  \item
  Модель с HF (Wakanda - Rus). 
\item
  100 Гб текстов, 200 tasks/2 cpu core/1 month (Kudos to University). Грязный перевод.
\item
  Части N-gram сохраняются. Достаточно чтобы обнаруживать перевод.
\item
  Подход с эмбеддингами возможен. Требуется перевод.
\item
  Чистовой перевод при помощи OpenAI. Бесплатные кредиты. Kudos.
\end{itemize}  
}
...
\begin{itemize}
  ...
  \item Порядка 9К внутреннего списывания.

  \item 5K переводов. В обратную сторону 50-100.
...
\begin{itemize}
  ...
  \item Простые классификаторы могут определять. 1-, 2-, 3-ngram. 
\href{https://x.com/hippopedoid/status/1804057603628012001}{https://x.com/hippopedoid/status/1804057603628012001} Dmitry Kobak 
  
  \item Наличие AI не означает плагиат или злодеяния.

  \item Моральные аспекты.
...
\frame {
  ...
  Есть датасет. Понятые. Численные данные. Другие страны.
...
\frame {
  \frametitle {А теперь о хорошем}

\begin{itemize}

\item  Данные можно использовать иначе. Например, для ускорения прогресса.
...
\item RAG на 100М документов? 

\item Текущие системы работают на 50-200К документов. (Writer.com, hackatons)

\pause

\item Только онкологии имеется 1M-3M статей в медицине и смежных областях, порядка 40К статей в год про рак (2025).

\item Помогает ли подорожник? Вредно ли кофе. Как вместить 1М документов в контекст LLM? 
...
\item Эмбеддинги - ресурсы. Датасет большой. х30-40 раз больше чем перевод на месяц.

\item Эмбеддинги разбухают, больше чем текст. KNN. 5-10B векторов.
