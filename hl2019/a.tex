...
Встретить 0.368 <<более вероятно>> $\textsuperscript{\textcopyright}$, чем 0.000001 (на 5-10 элементах).
}
...
Чтобы спуститься на уровень вниз - переберем алфавит (либо дополнительную структуру, хранящую возможные переходы).
...
Конструктор: (relative error (eps), confidence (delta))  $(0.0000001, 0.9999)$
...
Количество лайков - классический KV, либо $CountMinSketch(objectId)$.

\vspace{5mm}

Шардинг по $objectId, (userId, objectId)$
...
$BloomFilter$ для $(userId, itemId)$, смотрел ли вообще конкретный человек конкретное объявление. Глобальный. Это и есть количество уникальных. 

\vspace{5mm}

Инкрементируем просмотры в $CountMinSketch$, на каждый квант времени (час) - отдельный. 
Их можно складывать/вычитать, организовать $sliding\ window$. В качестве ключа использовать $itemId$ или $(userId, itemId)$.
